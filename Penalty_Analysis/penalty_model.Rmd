...
top_dhoe <- secondary_evaluation_tbl %>%
  top_n(percent_predicted, n = 20) %>%
  ggplot(aes(
    fct_reorder(def_player, percent_predicted),
    percent_predicted
  ...
  labs(
    ...
    y = "",
    title = "'Hey Ref, He's Holding!'",
    subtitle = "Top 20 in DHOE (Minimum 200 Routes Defended)"
  ) +
  coord_flip() +
  theme(
    text = element_text(size = 14, family = "Calibri"),
...
bottom_dhoe <- secondary_evaluation_tbl %>%
  top_n(-percent_predicted, n = 20) %>%
  ggplot(aes(
    fct_reorder(def_player, -percent_predicted),
    percent_predicted
  ...
  labs(
    ...
    subtitle = "Bottom 20 in DHOE, (Minimum 200 Routes Defended)"
  ) +
  geom_text(
    data = data.frame(x = 12.0579666924506,
                      y = 0.258663026411452,
